...
public final class LDAModelGenetrator {
    ...
    /**
     * Try to load matrix .dat files, otherwise check for deck folders and build
     * .dat, otherwise return false
     **/
        ...
        for (int t = 0; t < lda.size(); ++t) {
            ...
            while (topic.size() <= 40 && i < highRankVocabs.size()) {
                ...
                if (!card.getRules().getType().isBasicLand()) {
                    ...
                    System.out.println(
                            "[" + highRankVocabs.get(i).getLeft() + "," + highRankVocabs.get(i).getRight() + "],");
    ...
    public static List<Archetype> initializeFormat(GameFormat format) throws Exception {
        ...
        for (int t = 0; t < lda.getNumTopics(); ++t) {
            ...
            for (Deck deck : decks) {
                String name = deck.getName().replaceAll(".* Version - ", "")
                        .replaceAll(" \\((" + SUPPORTED_LDA_FORMATS + "), #[0-9]+\\)", "");
    ...
    private static <K, V> Map<K, V> sortByValue(Map<K, V> map) {
        ...
        list.sort((Comparator<Object>) (o1, o2) -> ((Comparable<V>) ((Map.Entry<K, V>) (o2)).getValue())
        ...
        for (PaperCard legend : legends) {
            for (Deck deck : decks) {
                ...
                if (deck.getCommanders().contains(legend)) {
                    // update the matrix by incrementing the connectivity count for each card in the
                    // deck
        ...
        for (PaperCard card : legends) {
            ...
            if (max > 0) {
                ...
                for (int k = 0; k < cardList.size(); k++) {
                    if (matrix[col][k] > 0) {
                        deckPool.add(
                                new AbstractMap.SimpleEntry<PaperCard, Integer>(integerCardMap.get(k), matrix[col][k]));
                    }
                }
        ...
        deck.getMain().toFlatList().stream()
                ...
                .forEach(pairCard -> {
                    try {
                        ...
                        // TODO: Pas sur de ce qui bloque ici
                        ne.printStackTrace();
                    }
                });
